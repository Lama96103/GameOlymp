...
function GetColor($pHome, $pGuest, $guest, $confirmed, $played){
    ...
    }else if($played == 1 && $confirmed == 0){
        return 'bg-warning';
    }else if($played == 0 && $confirmed == 0 && !($pHome == 0 && $pHome == 0)){
        return 'bg-danger';
    }else{
      return 'bg-dark';
    }
...
function GetRunning($link, $id){
    $run = "SELECT finished FROM tournaments where id = " . $id;
    $result = mysqli_query($link, $run);
